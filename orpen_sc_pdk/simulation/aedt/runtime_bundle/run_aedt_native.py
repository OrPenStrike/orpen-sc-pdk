"""Run an AEDT-native package generated by orpen_sc_pdk."""
...
def package_path(package_root, relative):
...
def cli_option_present(raw_args, *flags):
    for arg in raw_args:
        for flag in flags:
...
def resolve_requested_mode(args):
    requested = []
...
def run_config_path_for_mode(manifest, manifest_path, mode):
...
def apply_run_config(args, manifest, manifest_path, raw_args):
...
def empty_aedt_material_context():
    return {
        "schema_version": "aedt-material-context.v1",
        "material_condition": "cryogenic",
        "registry_hash": None,
        "layer_stack_hash": None,
        "bindings": [],
        "compiled_materials": [],
    }


def load_aedt_material_context(case, package_root):
...
def material_context_bindings(material_context):
    return list((material_context or {}).get("bindings") or [])


def material_context_compiled_materials(material_context):
    return list((material_context or {}).get("compiled_materials") or [])


def material_context_binding_by_layer_number(material_context):
    bindings = {}
...
def material_context_binding_for_object_name(name, material_context):
    text = str(name)
...
def material_context_material_for_row(row, material_context):
...
def ensure_aedt_project_materials(app, material_context, result_dir=None, *, allow_missing=False):
    ...
    if materials_manager is None:
        if allow_missing:
            return {
                ...
                "materials": [],
                "skipped": "app has no materials manager",
            }
...
def existing_or_new_aedt_material(materials_manager, name):
    material_obj = None
    try:
...
def apply_aedt_material_properties(material_obj, supported):
    property_names = (
        "permittivity",
        "permeability",
...
def preflight_payload(args, manifest, output_roots):
    ...
    return {
        ...
        "hpc": {
            ...
            "resolved_resource": hpc_resolved_payload(
...
def resolve_output_roots(args, package_root):
    run_root = infer_run_root(package_root)
    if args.results_root:
        results_root = Path(args.results_root).resolve()
        results_source = "cli"
    elif run_root is not None:
        results_root = run_root / "results" / "aedt"
        results_source = "run_folder"
    else:
        results_root = package_root / "results"
        results_source = "package_fallback"

    if args.logs_root:
        logs_root = Path(args.logs_root).resolve()
        logs_source = "cli"
    elif run_root is not None:
        logs_root = run_root / "logs" / "aedt"
        logs_source = "run_folder"
    else:
        logs_root = package_root / "logs"
        logs_source = "package_fallback"
...
def infer_run_root(package_root):
    package_root = Path(package_root).resolve()
    if package_root.name == "aedt_native" and package_root.parent.name == "exports":
...
def finalize_aedt_session(args, logs_root, manifest):
...
def q2d_geometry_mode(recipe):
    ...
    if mode not in {"hfss_section", "native_2d"}:
...
def load_q2d_source_metadata(case, package_root):
    relative = case.get("source_metadata")
    if not relative:
        raise RuntimeError(
            "native_2d Q2D geometry mode requires a source_metadata sidecar "
            f"for case {case['id']!r}"
        )
    payload = read_json(package_path(package_root, relative))
    if not isinstance(payload, dict):
        raise RuntimeError(
            "native_2d Q2D geometry mode requires source_metadata to be a JSON object"
        )
    return payload


def q2d_native_2d_parameters(source_metadata):
    parameters = dict(source_metadata.get("parameters") or {})
    for key in (
        "case_kind",
        "cpw_left_gap_um",
        "cpw_width_um",
        ...
        if key not in parameters and key in source_metadata:
            parameters[key] = source_metadata[key]
    case_kind = str(parameters.get("case_kind") or source_metadata.get("case_kind") or "")
    if "flip_chip" not in case_kind:
        raise RuntimeError(
            "native_2d Q2D geometry currently supports flip-chip CPW cases only; "
    ...
    if missing:
        raise RuntimeError(
            "native_2d Q2D geometry source_metadata is missing required "
            f"CPW parameter(s): {missing}"
        )
    numeric = {}
    for key in required:
        try:
            ...
            raise RuntimeError(
                f"native_2d Q2D geometry parameter {key!r} must be numeric; got {parameters[key]!r}"
            ) from exc
    positive = ("cpw_left_gap_um", "cpw_width_um", "cpw_right_gap_um", "flip_chip_gap_um")
    invalid = [key for key in positive if numeric[key] <= 0.0]
    if invalid:
        raise RuntimeError(f"native_2d Q2D geometry parameters must be positive: {invalid}")
    return {"case_kind": case_kind, **numeric}


def q2d_native_mapping_layers_by_name(mapping_payload):
    layers = {}
    for row in mapping_payload.get("layers") or mapping_payload.get("gds_import_layers") or []:
        layer_name = str(row.get("layer_name") or "").strip()
        if layer_name:
            layers[layer_name] = row
    return layers


def q2d_native_bindings_by_layer_name(material_context):
    bindings = {}
    for binding in material_context_bindings(material_context):
        layer_name = str(binding.get("layer_name") or "").strip()
        if layer_name:
            bindings[layer_name] = binding
    return bindings


def q2d_native_layer_record(layer_name, layers_by_name, bindings_by_name):
    row = layers_by_name.get(layer_name)
    binding = bindings_by_name.get(layer_name)
    if row is None:
        raise RuntimeError(f"native_2d Q2D geometry requires layer mapping row {layer_name!r}")
    if binding is None:
        raise RuntimeError(f"native_2d Q2D geometry requires material binding {layer_name!r}")
    return row, binding


def q2d_native_float_field(payload, key, *, label):
    value = payload.get(key)
    if value in (None, ""):
        raise RuntimeError(f"native_2d Q2D geometry requires {label} field {key!r}")
    try:
        return float(value)
    except (TypeError, ValueError) as exc:
        raise RuntimeError(
            f"native_2d Q2D geometry field {label}.{key} must be numeric; got {value!r}"
        ) from exc


def q2d_native_layer_bounds(row, *, layer_name):
    return (
        q2d_native_float_field(row, "bbox_ymin_um", label=layer_name),
        q2d_native_float_field(row, "bbox_ymax_um", label=layer_name),
    )


def q2d_native_z_span(row, binding, *, layer_name):
    zmin_payload = binding if binding.get("zmin_um") not in (None, "") else row
    thickness_payload = binding if binding.get("thickness_um") not in (None, "") else row
    zmin = q2d_native_float_field(zmin_payload, "zmin_um", label=layer_name)
    thickness = q2d_native_float_field(
        thickness_payload,
        "thickness_um",
        label=layer_name,
    )
    if thickness <= 0.0:
        raise RuntimeError(
            f"native_2d Q2D geometry layer {layer_name!r} must have positive thickness"
        )
    return zmin, thickness


def q2d_native_material(row, binding, *, fallback="pec"):
    material = binding.get("aedt_material_name") or row.get("material") or fallback
    return normalize_aedt_material(material)


def q2d_native_rectangle_record(
    *,
    name,
    kind,
    layer_name,
    ...
    if width <= 0.0 or height <= 0.0:
        raise RuntimeError(
            "native_2d Q2D geometry rectangle has non-positive size: "
...
def q2d_native_marker_layer(marker):
    return str(marker.get("layer_stack_layer_name") or "").strip()


def q2d_native_signal_marker(conductor_groups, *, layer_name, assignment_name):
    for group in conductor_groups:
        if group.get("assignment_name") != assignment_name:
            continue
        if group.get("conductor_type") != "Signal Line":
            continue
        markers = [
            marker
            for marker in group.get("markers", [])
            if q2d_native_marker_layer(marker) == layer_name
        ]
        if len(markers) != 1:
            raise RuntimeError(
                "native_2d Q2D geometry expected exactly one signal marker for "
                f"{assignment_name!r} on {layer_name!r}; got "
                f"{[marker.get('name') for marker in markers]}"
            )
        return markers[0]
    raise RuntimeError(
        "native_2d Q2D geometry requires a Signal Line assignment "
        f"{assignment_name!r} on {layer_name!r}"
    )


def q2d_native_ground_markers(conductor_groups, *, layer_name):
    markers = []
    for group in conductor_groups:
        if group.get("assignment_name") != "Ground":
            continue
        if group.get("conductor_type") != "Reference Ground":
            continue
        markers.extend(
            marker
            for marker in group.get("markers", [])
    ...
    if len(markers) < 2:
        raise RuntimeError(
            "native_2d Q2D geometry expected at least two Ground markers on "
            f"{layer_name!r}; got {[marker.get('name') for marker in markers]}"
        )
    return markers


def q2d_native_marker_center_y(marker):
    for key in ("center_y_um", "source_y_um"):
        value = marker.get(key)
        if value in (None, ""):
            continue
        return float(value)
    raise RuntimeError(
        f"native_2d Q2D geometry marker is missing center_y_um/source_y_um: {marker.get('name')!r}"
    )


def build_q2d_native_2d_geometry_plan(
    *,
    case,
    recipe,
    mapping_payload,
    ...
    parameters = q2d_native_2d_parameters(source_metadata)
    layers_by_name = q2d_native_mapping_layers_by_name(mapping_payload)
    bindings_by_name = q2d_native_bindings_by_layer_name(material_context)
    required_layers = ("D0_SUBSTRATE", "D1_SUBSTRATE", "D0_TOP_M1", "D1_BOTTOM_M1")
    for layer_name in required_layers:
        q2d_native_layer_record(layer_name, layers_by_name, bindings_by_name)

    rectangles = []
    assignments = {"Ground": [], "Trace1": [], "Trace2": []}
    marker_summaries = {}
    for layer_name in ("D0_SUBSTRATE", "D1_SUBSTRATE"):
        row, binding = q2d_native_layer_record(layer_name, layers_by_name, bindings_by_name)
        x_min, x_max = q2d_native_layer_bounds(row, layer_name=layer_name)
        y_min, thickness = q2d_native_z_span(row, binding, layer_name=layer_name)
        rectangles.append(
            q2d_native_rectangle_record(
                name=f"Native2D_{layer_name}",
                kind="substrate",
                layer_name=layer_name,
                x_min=x_min,
                x_max=x_max,
                ...
                material=q2d_native_material(row, binding, fallback="Silicon"),
            )
        )

    cpw_width = parameters["cpw_width_um"]
    left_gap = parameters["cpw_left_gap_um"]
    right_gap = parameters["cpw_right_gap_um"]
    signal_specs = (
        ("D0_TOP_M1", "Trace1"),
        ("D1_BOTTOM_M1", "Trace2"),
    )
    for layer_name, assignment_name in signal_specs:
        row, binding = q2d_native_layer_record(layer_name, layers_by_name, bindings_by_name)
        signal_marker = q2d_native_signal_marker(
            conductor_groups,
            layer_name=layer_name,
            assignment_name=assignment_name,
        )
        ground_markers = q2d_native_ground_markers(conductor_groups, layer_name=layer_name)
        signal_center = q2d_native_marker_center_y(signal_marker)
        x_min, x_max = q2d_native_layer_bounds(row, layer_name=layer_name)
        y_min, thickness = q2d_native_z_span(row, binding, layer_name=layer_name)
        y_max = y_min + thickness
        material = q2d_native_material(row, binding, fallback="pec")
        signal_left = signal_center - cpw_width / 2.0
        signal_right = signal_center + cpw_width / 2.0
        left_ground_right = signal_left - left_gap
        right_ground_left = signal_right + right_gap
        layer_rectangles = [
            q2d_native_rectangle_record(
                name=f"Native2D_{layer_name}_Ground_Left",
                kind="conductor",
                layer_name=layer_name,
                x_min=x_min,
                x_max=left_ground_right,
                y_min=y_min,
                y_max=y_max,
                material=material,
                assignment_name="Ground",
            ...
            q2d_native_rectangle_record(
                name=f"Native2D_{layer_name}_{assignment_name}",
                kind="conductor",
                layer_name=layer_name,
                x_min=signal_left,
                x_max=signal_right,
            ...
            q2d_native_rectangle_record(
                name=f"Native2D_{layer_name}_Ground_Right",
                kind="conductor",
                layer_name=layer_name,
                x_min=right_ground_left,
                x_max=x_max,
                y_min=y_min,
                y_max=y_max,
                material=material,
        ...
        for rectangle in layer_rectangles:
            assignment = rectangle.get("assignment_name")
            if assignment:
                assignments.setdefault(assignment, []).append(rectangle["name"])
        assignments.setdefault(assignment_name, [])
        marker_summaries[assignment_name] = {
            "signal_marker": signal_marker.get("name"),
            "ground_markers": [marker.get("name") for marker in ground_markers],
        }

    return {
        "schema_version": "aedt-q2d-native-2d-geometry-plan.v1",
...
def create_q2d_native_2d_rectangles(q2d, geometry_plan):
    created = []
    for rectangle in geometry_plan["rectangles"]:
        ...
        if obj is None:
            raise RuntimeError(f"Failed to create native_2d Q2D rectangle {rectangle['name']!r}")
        try:
            obj.material_name = rectangle["material"]
        except Exception:
            pass
        created.append(getattr(obj, "name", rectangle["name"]))
    return created


def q2d_native_2d_geometry_inventory(q2d, geometry_plan):
    records = []
    for rectangle in geometry_plan["rectangles"]:
        name = rectangle["name"]
        ...
        records.append(
            {
                ...
                "planned_bounds": rectangle.get("bounds"),
                "assignment_name": rectangle.get("assignment_name"),
                "conductor_type": rectangle.get("conductor_type"),
            }
        )
    return records


def q2d_native_existing_plan_matches(q2d, geometry_plan):
    existing_names = set(modeler_object_names(q2d))
    planned_names = {rectangle["name"] for rectangle in geometry_plan["rectangles"]}
    return planned_names.issubset(existing_names)


def ensure_q2d_native_2d_geometry(
    case,
    recipe,
    manifest,
    package_root,
    ...
    source_metadata = load_q2d_source_metadata(case, package_root)
    geometry_plan = build_q2d_native_2d_geometry_plan(
        case=case,
        recipe=recipe,
    ...
    ensure_design_modeler_units(q2d, recipe, "q2d_native_2d")
    ...
    if q2d_native_existing_plan_matches(q2d, geometry_plan) and not args.force_rebuild:
        inventory = q2d_native_2d_geometry_inventory(q2d, geometry_plan)
        write_json(result_dir / "q2d_native_2d_geometry_plan.json", geometry_plan)
        write_json(result_dir / "q2d_native_2d_geometry_inventory.json", inventory)
        return (
            q2d,
            geometry_plan,
            inventory,
            stage_record(
                "q2d_native_2d_geometry",
                "skipped",
                rectangle_count=len(geometry_plan["rectangles"]),
    ...
    existing_names = [
        ...
        if str(name).casefold() not in {"region", "vacuum"}
    ]
    if existing_names and not args.force_rebuild:
        raise RuntimeError(
            "Existing Q2D design geometry does not match native_2d plan. "
            "Rerun with --force-rebuild before switching geometry modes. "
            f"Existing objects: {existing_names}"
        )
    created = create_q2d_native_2d_rectangles(q2d, geometry_plan)
    inventory = q2d_native_2d_geometry_inventory(q2d, geometry_plan)
    write_json(result_dir / "q2d_native_2d_geometry_plan.json", geometry_plan)
    write_json(result_dir / "q2d_native_2d_geometry_inventory.json", inventory)
    return (
        ...
        stage_record(
            "q2d_native_2d_geometry",
            "created",
            rectangle_count=len(created),
            material_summary=material_summary,
        ),
    )


def assign_q2d_native_2d_conductor_groups(q2d, geometry_plan, conductor_groups, recipe):
    assignments = []
    planned_by_assignment = {}
    for rectangle in geometry_plan["rectangles"]:
        assignment_name = rectangle.get("assignment_name")
        if not assignment_name:
            continue
        planned_by_assignment.setdefault(assignment_name, []).append(rectangle["name"])
    delete_boundaries_by_name(q2d, {group["assignment_name"] for group in conductor_groups})
    units = recipe_modeler_units(recipe)
    for group in conductor_groups:
        assignment_name = group["assignment_name"]
        objects = sorted(set(planned_by_assignment.get(assignment_name, [])))
        if not objects:
            raise RuntimeError(
                "native_2d Q2D geometry has no planned objects for conductor "
                f"assignment {assignment_name!r}"
            )
        assignment_objects = [q2d.modeler.get_object_from_name(name) for name in objects]
        if any(obj is None for obj in assignment_objects):
            raise RuntimeError(f"native_2d Q2D assignment objects were not found: {objects}")
        boundary = q2d.assign_single_conductor(
            assignment_objects,
            name=assignment_name,
            conductor_type=q2d_conductor_type_for_aedt(group["conductor_type"]),
            units=units,
        )
        assignments.append(
            {
                ...
                "objects": objects,
                "boundary": getattr(boundary, "name", assignment_name),
            }
        )
    return {
...
def run_q2d_incremental_workflow(case, recipe, manifest, package_root, result_dir, log_dir, args):
    ...
    try:
        ...
        section_objects = []
        q2d_section_objects = []
        native_geometry_plan = None
        native_geometry_inventory = []
        if geometry_mode == "native_2d":
            if assignment_source != "q2d_conductors":
                raise RuntimeError(
                    "native_2d Q2D geometry requires assignment_source='q2d_conductors'"
                )
            ...
            q2d, native_geometry_plan, native_geometry_inventory, record = (
                ensure_q2d_native_2d_geometry(
                    case,
                    recipe,
                    manifest,
                    package_root,
        ...
        region_summary = create_q2d_region(q2d, recipe)
        detection["stages"].append(stage_record("region", "repaired", **region_summary))
        if geometry_mode == "native_2d":
            assignment_summary = assign_q2d_native_2d_conductor_groups(
                q2d,
                native_geometry_plan,
                conductor_groups,
                recipe,
            )
            ...
            write_json(result_dir / "assignment_summary.json", assignment_summary)
        else:
        ...
        write_json(
            result_dir / "simulation_metadata.json",
            {
                ...
                "native_2d_rectangle_count": len(
                    native_geometry_plan.get("rectangles", []) if native_geometry_plan else []
                ),
                "native_2d_inventory_count": len(native_geometry_inventory),
                "modeler_units": recipe_modeler_units(recipe),
                "material_policy": recipe.get("material_policy", {}),
                "aedt_material_context": {
...
def layer_number_from_object_name(name):
    import re
...
def worker_project_path(args, manifest, manifest_path, case_id, recipe_id, results_root):
    ...
    if args.worker_project_root:
        ...
        return project_root / token / f"{project_name}.aedt"
    return Path(results_root) / case_id / recipe_id / "aedt_project" / f"{project_name}.aedt"
...
def resolve_parallel_max_workers(manifest, args):
    resource = apply_hpc_cli_overrides(manifest_hpc_resource(manifest), args)
...
def parallel_worker_command(
    ...
    *,
    ...
    if args.grpc_port is not None:
        command.extend(["--grpc-port", str(args.grpc_port)])
...
def run_parallel_worker_subprocess(command, stdout_path):
...
class ParallelProgressReporter:
    def __init__(
        ...
        *,
        ...
        self._dynamic = self.mode in {"auto", "stream"} and sys.stdout.isatty()
    ...
    def snapshot(self):
        ...
        return {
            ...
            "eta_seconds": eta,
            "stage_counts": parallel_stage_counts(self.progress_paths),
        }
...
def format_parallel_progress_line(snapshot):
    ...
    parts = [
        f"AEDT parallel [{bar}] {done}/{total} done",
        f"running<={snapshot.get('running_limit', 0)}",
...
def parallel_axis_coverage(manifest, package_root, pairs):
    ...
    for case, _recipe in pairs:
        ...
        parameters = case_parameters_from_manifest_case(package_root, case_row)
...
def case_parameters_from_manifest_case(package_root, case):
    source_metadata = case.get("source_metadata")
    if not source_metadata:
        return {}
    payload = read_json(Path(package_root) / source_metadata) or {}
...
def run_parallel_package(manifest, manifest_path, args):
    ...
    max_workers = resolve_parallel_max_workers(manifest, args)
    worker_project_root = (
    ...
    resource = hpc_resolved_payload(apply_hpc_cli_overrides(manifest_hpc_resource(manifest), args))
    write_json(
        parent_log_root / "aedt_parallel_preflight.json",
        {
            ...
            "worker_project_default": (
                "results/aedt/<point_slug>/<recipe_id>/aedt_project/<project>.aedt"
    ...
    for case, recipe in selected_pairs:
        ...
        skip_status = should_skip_recipe_for_resume(result_dir, log_dir, recipe, args)
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for case, recipe in pending:
                ...
                future = executor.submit(run_parallel_worker_subprocess, command, stdout_path)
                futures[future] = (case, recipe, worker_log_root)

            if pending:
                progress.event_line(f"Queued {len(pending)} AEDT workers | running<={max_workers}")
            ...
            while remaining_futures:
                ...
                if not done:
                    ...
                    continue
                for future in done:
                    case, recipe, worker_log_root = futures[future]
                    result_dir = results_root / case["id"] / recipe["id"]
                    log_dir = logs_root / case["id"] / recipe["id"]
                    try:
                    ...
                    if args.mode == "import":
                        completion = {
                            "completion_status": (
                                "import_complete"
                                if worker_result.get("returncode") == 0 and worker_project.is_file()
                                else "import_failed"
                            ),
                    ...
                    status = (
                        ...
                        and completion.get("completion_status") in {"complete", "import_complete"}
                        else "failed"
                    )
                    if status == "failed":
                        package_failed = True
                    summary_rows.append(
                        {
                            ...
                            "worker_stdout_log": worker_result.get("stdout_log"),
                            "worker_project_path": str(worker_project),
                            "worker_project_exists": worker_project.is_file(),
                            "elapsed_seconds": worker_result.get("elapsed_seconds"),
                        }
                    )
                    ...
                    if status == "failed":
                        progress.event_line(
                            f"AEDT worker failed: {case['id']} / {recipe['id']} "
                            f"returncode={worker_result.get('returncode')}"
                        )
                    progress.maybe_render()
    finally:
        progress.finish()
...
def should_skip_recipe_for_resume(result_dir, log_dir, recipe, args):
    if args.mode != "solve":
        return None
    ...
    if recipe.get("type") != "q2d_extraction":
        return None
    status = q2d_solve_completion_status(result_dir, log_dir, recipe)
...
def run_package(manifest, manifest_path, args):
    ...
    try:
        ...
        finalize_aedt_session(args, root_log_dir, manifest)
...
def main():
    ...
    parser.add_argument(
        "--manifest",
        default=str(Path(__file__).resolve().parents[1] / "manifest.yaml"),
    )
    ...
    parser.add_argument(
        ...
        default="auto",
        help="Display parent-side point progress for parallel solve mode.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--results-root",
        default=None,
        help="Optional results root. Defaults to the run-level results/aedt directory.",
    )
    parser.add_argument(
        "--logs-root",
        default=None,
        help="Optional logs root. Defaults to the run-level logs/aedt directory.",
    )
    raw_args = sys.argv[1:]
    ...
    if should_run_parallel_parent(args):
        run_parallel_package(manifest, manifest_path, args)
        return
