...
| Surface-Q, EPR, capacitance, eigenmode, material, and driven-result report parsing | `gsim` | Keep reusable result loaders and report schemas upstream; local `gsim` now has indexed domain-energy, surface-Q, port-EPR, electrostatic matrix, effective domain material summaries, configured dielectric interface summaries, Eigenmode modal/history primitives, and a composed Eigenmode report bundle. |
