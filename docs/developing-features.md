...
| P4 | Route B finite-metal Surface EPR interface groups | `gsim` | M | Yes | full 3D conductor faces lower to PEC shell surfaces with generated MS/MA interface groups, plus 50 nm planar band/core groups where supported | P1, P3 |
...
| P7 | Martinis ribbon Surface EPR MS notebooks | `orpen-sc-pdk` | S | No | handoff and local notebooks select the Route B MS bottom total/band/core groups from `gsim` outputs | P4 |
...
| MA/SA reporting and non-planar geodesic inset bands | Route B now exposes generated MA top/sidewall groups for inspection, but the public notebooks intentionally activate only the Martinis MS preset. |
...
|---|---|---|---|
| Route B finite-metal shell Surface EPR, MS-bottom selection | Lets `gsim` discover finite-metal shell interfaces and split supported MS/MA top and sidewall surfaces into total, 50 nm band, and core groups for Surface EPR postprocessing. | `notebooks/src/public_surface_epr_ribbon_capacitor_workflow.py` and `notebooks/src/public_surface_epr_ribbon_capacitor_local_workflow.py` | MA/SA reporting and non-planar geodesic inset bands are deferred; this slice only activates the public MS preset. |
| Route B finite-metal shell Surface EPR local test | Uses the same generated interface catalog and MS-bottom postprocessing selection while exposing a direct `sim.run_local()` Run Stage. | `notebooks/src/public_surface_epr_ribbon_capacitor_local_workflow.py` | This is still surface-based EPR; it does not claim true metal-volume/ohmic loss. |
