...
| P4 | Route B finite-metal Surface EPR interface groups | `gsim` | M | Yes | full 3D conductor faces lower to PEC shell surfaces with generated MS/MA interface groups plus SA-top seed groups for public comparison rows | P1, P3 |
...
| P7 | Martinis ribbon Surface EPR notebooks | `orpen-sc-pdk` | S | No | handoff B keeps the Route B MS-bottom public slice while local B selects public MS/MA/SA presets through `sim.set_surface_epr(...)` | P4 |
...
| MA/SA process calibration | Route C can activate public MS/MA/SA Surface EPR rows, but calibrated process-default MA/SA loss policy remains a later public-data decision. |
...
|---|---|---|---|
| Surface EPR A/B/C representation notebooks | Shows the public Martinis ribbon capacitor through independent A, B, and C notebook presentations using `sim.set_surface_epr(...)`. | `notebooks/src/public_surface_epr_ribbon_capacitor_representation_a_workflow.py`, `notebooks/src/public_surface_epr_ribbon_capacitor_workflow.py`, and `notebooks/src/public_surface_epr_ribbon_capacitor_representation_c_workflow.py` | Notebooks select representation, inset margins, and public interface presets; `gsim` owns generated interfaces and Palace postprocessing. |
| Route C retained-volume Surface EPR inset validation | Keeps the 3D metal volume, validates generated MS/MA/SA child inset physical groups against the mesh file, and checks logical total postprocessing rows before handoff. | `notebooks/src/public_surface_epr_ribbon_capacitor_representation_c_workflow.py` and `tests/test_surface_epr_representation_c_workflow.py` | C notebooks demonstrate public MS/MA/SA rows; calibrated process-default MA/SA policy still belongs to public material data review. |
| Route B finite-metal shell Surface EPR | Lets `gsim` discover finite-metal shell interfaces and split supported MS/MA/SA rows into cumulative Surface EPR postprocessing config. | `notebooks/src/public_surface_epr_ribbon_capacitor_workflow.py` and `notebooks/src/public_surface_epr_ribbon_capacitor_representation_b_local_workflow.py` | The handoff B notebook remains MS-bottom focused; local B activates public MS/MA/SA presets for comparison. |
| Route B finite-metal shell Surface EPR local test | Uses the same generated interface catalog with MS/MA/SA postprocessing selection while exposing a direct `sim.run_local()` Run Stage. | `notebooks/src/public_surface_epr_ribbon_capacitor_representation_b_local_workflow.py` | This is still surface-based EPR; it does not claim true metal-volume/ohmic loss. |
...
| Dielectric interface preset schema for loss records | `orpen-sc-pdk` schema with `gsim` assignment helpers | Keep public interface preset records validated, explicitly sourced, and JSON-safe in the PDK while `gsim` owns Palace `DielectricInterfaceSpec`, interface catalog assignment, material-kind classification, material resolution, config emission, and report joins. Local Route B and Route C can activate public MS/MA/SA comparison rows; calibrated automatic default-selection rules remain deferred until public process scope is accepted. |
