...
import orpen_sc_pdk
from orpen_sc_pdk import PDK, cells, config, models, samples, tech
...
def test_qpdk_style_public_import_surface() -> None:
    ...
    assert cells.as_launcher
    assert cells.as_interdigital_capacitor
...
def test_gdsfactory_get_component_works_after_activation() -> None:
    ...
    assert gf.get_component("as_launcher").ports
    assert gf.get_component("as_interdigital_capacitor").ports
    assert gf.get_component("quarter_wave_resonator").ports


def test_public_samples_generate_components() -> None:
    functions = orpen_sc_pdk.get_sample_functions()

    assert "orpen_sc_pdk.samples.all_cells.all_public_cells" in functions
